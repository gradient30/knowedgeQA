...
| P2 | Collaboration and governance support files, comments, likes, favorites, notifications, auth, profile, and metrics. | File upload, `POST /api/v1/knowledge/articles/{id}/comments`, `POST /api/v1/knowledge/articles/{id}/like`, `POST /api/v1/knowledge/articles/{id}/favorite`, `GET /api/v1/knowledge/metrics`, notification admin, auth/profile routes. | `/files`, `/admin/notifications`, `/profile`, `/login` build successfully; runtime gate verifies comment, like, favorite, and metrics. | `node scripts/verify-runtime-acceptance.js`, `pnpm build` | Full stack | Passed with warnings |
...
- Backend full regression: `python -m pytest tests/ --cov=app -q` -> 56 passed, 67% coverage.
...
| Backend full regression | `python -m pytest tests/ --cov=app -q` | Passed: 56 tests, 67% coverage. |
