...
- Backend full regression: `python -m pytest tests/ --cov=app -q` -> 58 passed, 68% coverage.
...
6. Confirm admin review, file upload, notification settings, profile flows, and user/team stats still build and route.
...
| Backend full regression | `python -m pytest tests/ --cov=app -q` | Passed: 58 tests, 68% coverage. |
