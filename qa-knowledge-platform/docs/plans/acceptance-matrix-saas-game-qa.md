...
| P2 | Collaboration and governance support authenticated evidence files, comments, likes, favorites, notifications, auth, profile, audit logs, real metrics, and administrator user governance. | Authenticated file upload/list/download/delete, private file owner/admin access, one-time auth tokens for email verification/password reset, notification settings/templates/preview/test-email/logs, `POST /api/v1/knowledge/articles/{id}/comments`, `POST /api/v1/knowledge/articles/{id}/like`, `POST /api/v1/knowledge/articles/{id}/favorite`, `GET /api/v1/knowledge/metrics`, `GET /api/v1/users/stats`, `GET /api/v1/teams/{id}/stats`, `GET /api/v1/audit/logs`, auth/profile routes, `GET /api/v1/users/`, `GET /api/v1/users/{id}`, `PUT /api/v1/users/{id}/role`, `PUT /api/v1/users/{id}/status`. | `/files`, `/admin/notifications`, `/profile`, `/login` build successfully; runtime gate verifies auth token guards, authenticated file evidence flow, notification admin flow, comment, like, favorite, metrics, and audit flow; real browser E2E verifies login, file upload, and admin notification logs through the UI. | `python -m pytest tests/test_auth.py tests/test_files.py tests/test_notifications.py tests/test_users.py tests/test_audit_api.py tests/test_no_placeholder_api.py -q`, `node scripts/verify-runtime-acceptance.js`, `npx --yes --package playwright node scripts/verify-e2e-real-acceptance.js`, `pnpm build` | Full stack | Passed |
...
- Backend full regression: `python -m pytest tests/ --cov=app -q` -> 76 passed, 72% coverage, warning-free.
- Database migration graph: `poetry run alembic heads` -> single Alembic release head `20260628_user_tokens`; fresh empty database upgrade with `DATABASE_URL=postgresql+asyncpg://... poetry run alembic upgrade head` -> passed.
...
- Runtime Docker acceptance: `node scripts/verify-runtime-acceptance.js` -> validates backend health, invalid auth token rejection, SaaS/Game seed data, authenticated file upload/list evidence flow, notification admin settings/templates/preview/test-email/logs, knowledge write flow with linked evidence file, comment, like, favorite, metrics, audit flow, tool rating/favorite/usage flow, news source governance, source-backed intelligence flow, source-backed news summary, and frontend routes on the integrated stack.
...
- Real browser E2E acceptance: `npx --yes --package playwright node scripts/verify-e2e-real-acceptance.js` -> validates admin login, SaaS/Game knowledge and tool creation through the UI, authenticated file upload and management list, news filtering, and notification test-email logs with real browser interactions.
...
3. Upload evidence as a logged-in user, confirm private files are only available to the owner or an administrator, and link the file to a knowledge article.
4. Rate or favorite a game performance tool from `/tools`.
5. Publish a QA trend item from the news review workflow.
6. Search and filter knowledge, tools, and news by business domain and tag.
7. Confirm admin review, notification settings/templates/logs, audit flow, profile flows, and user/team stats still build and route.
...
| Runtime Docker acceptance | `node scripts/verify-runtime-acceptance.js` | Passed: health, invalid auth token rejection, SaaS/Game APIs, authenticated file upload/list evidence flow, notification settings/templates/preview/test-email/logs, knowledge create with evidence file/approve/comment/like/favorite/metrics/update/search/delete, audit flow for knowledge/news/source changes, tool create/delete/rate/favorite/usage, news source CRUD, news publish/reject, intelligence flow, and core frontend routes. |
...
| Real browser E2E acceptance | `npx --yes --package playwright node scripts/verify-e2e-real-acceptance.js` | Passed: admin login, knowledge/tool UI create flows, authenticated file upload/list, news domain filtering, and notification test email logs. |
...
| Backend full regression | `python -m pytest tests/ --cov=app -q` | Passed: 76 tests, 72% coverage, warning-free. |
| Database migration graph | `poetry run alembic heads`; run `poetry run alembic upgrade head` against a fresh empty database. | Passed: single Alembic release head `20260628_user_tokens`; fresh empty database upgrade passed. |
