...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        throw 'Status command unexpectedly succeeded without Docker on PATH.'
    }
    Assert-Contains ($statusOutput -join "`n") 'Docker 未安装或未加入 PATH' 'Status output should explain that Docker is missing.'
} finally {
    $env:PATH = $oldPath
}
