...
from app.modules.users.schemas import (
    ...
    PasswordChange, UserDataExport, TeamCreate, TeamUpdate, TeamResponse, 
    TeamInvitation, TeamMemberUpdate, TeamStats, TeamMemberInfo
)
from app.modules.users.tasks import send_verification_email, send_password_reset_email, send_welcome_email, send_email_change_verification
...
class AuthService:
    ...
    async def update_user_profile(self, user_id: UUID, update_data: UserUpdate) -> UserResponse:
        """更新用户资料"""
    ...
    async def request_email_change(self, user_id: UUID, new_email: str, password: str) -> bool:
        """请求修改邮箱"""
        # 获取用户信息
        user = await self.db.execute(
            select(User).where(User.id == user_id)
    ...
    async def get_user_stats(self, user_id: UUID) -> UserStats:
        """获取用户统计信息"""
        # TODO: 实现真实的统计查询，这里先返回模拟数据
        # 在后续任务中会实现文章、评论等模块后，这里会查询真实数据
    ...
    async def export_user_data(self, user_id: UUID, export_request: UserDataExport) -> Dict[str, Any]:
        """导出用户数据"""
        data = {}
        ...
        if export_request.include_articles:
            ...
            ] if export_request.format == "json" else []
        ...
        data["statistics"] = {
            ...
            "account_age_days": (
                ...
                if user and user.created_at else 0
            ),
    ...
    # 团队管理方法
    async def create_team(self, team_data: TeamCreate, creator_id: UUID) -> TeamResponse:
        """创建团队"""
        # 创建新团队
        db_team = Team(
            name=team_data.name,
    ...
    async def update_team(self, team_id: UUID, update_data: TeamUpdate, user_id: UUID) -> TeamResponse:
        """更新团队信息"""
        # 检查用户是否有权限更新团队
        team = await self.db.execute(
        ...
        if not user or (team.leader_id != user_id and user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]):
            raise HTTPException(
    ...
    async def invite_team_member(self, team_id: UUID, invitation: TeamInvitation, inviter_id: UUID) -> bool:
        """邀请团队成员"""
        # 检查团队是否存在
        team = await self.db.execute(
            select(Team).where(Team.id == team_id)
        ...
        if not inviter or (team.leader_id != inviter_id and inviter.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="权限不足"
            )
    ...
    async def remove_team_member(self, team_id: UUID, member_id: UUID, remover_id: UUID) -> bool:
        """移除团队成员"""
        # 检查团队是否存在
        team = await self.db.execute(
            select(Team).where(Team.id == team_id)
        ...
        # 检查操作者权限
        remover = await self.db.execute(
            select(User).where(User.id == remover_id)
        )
        remover = remover.scalar_one_or_none()
        
        if not remover or (team.leader_id != remover_id and remover.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="权限不足"
            )
    ...
    async def update_team_member_role(self, team_id: UUID, member_id: UUID, role_update: TeamMemberUpdate, updater_id: UUID) -> bool:
        """更新团队成员角色"""
        # 检查团队是否存在
        team = await self.db.execute(
            select(Team).where(Team.id == team_id)
        ...
        # 检查操作者权限
        updater = await self.db.execute(
            select(User).where(User.id == updater_id)
        )
        updater = updater.scalar_one_or_none()
        
        if not updater or (team.leader_id != updater_id and updater.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="权限不足"
            )
    ...
    async def get_team_stats(self, team_id: UUID) -> TeamStats:
        ...
        # TODO: 在后续任务中实现文章和工具统计
        # 这里先返回模拟数据
        
        return TeamStats(
            total_members=total_members,
            active_members=active_members,
            total_articles=0,  # TODO: 实现真实统计
            total_tools=0,     # TODO: 实现真实统计
            recent_activity_count=0  # TODO: 实现真实统计
        )
