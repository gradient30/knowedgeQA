...
async def request_password_reset(
    reset_request: PasswordReset, auth_service: AuthService = Depends(get_auth_service)
):
    """
    请求密码重置
...
async def request_email_change(
    ...
    await auth_service.request_email_change(
        current_user.id, email_change_data.new_email, email_change_data.password
    )
...
async def export_user_data(
    ...
    if export_request.format.lower() == "csv":
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=user_data_{current_user.username}_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
    else:
        # JSON 格式导出
...
async def get_export_stats(
    ...
    estimated_sizes = {
        "profile": "< 1KB",
        "articles": f"约 {stats.articles_count * 2}KB"
        if stats.articles_count > 0
        else "< 1KB",
        "comments": f"约 {stats.comments_count * 0.5}KB"
    ...
    return {
        ...
        "export_limit": {"daily": 5, "monthly": 30},
    }
...
async def get_team(
    ...
    if current_user.team_id != team_id and current_user.role not in [
        UserRole.ADMIN,
...
@teams_router.delete(
    "/teams/{team_id}/members/{member_id}", status_code=status.HTTP_200_OK
)
...
@teams_router.put(
    "/teams/{team_id}/members/{member_id}/role", status_code=status.HTTP_200_OK
)
...
async def update_team_member_role(
    ...
    """更新团队成员角色"""
    await auth_service.update_team_member_role(
        team_id, member_id, role_update, current_user.id
    )
...
async def get_current_user_team(
    ...
    if not current_user.team_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="用户未加入任何团队")
