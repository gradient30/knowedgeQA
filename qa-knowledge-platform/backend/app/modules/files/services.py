...
class FileService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.file_type_mapping = {
            ...
            '.svg': FileType.IMAGE, '.tiff': FileType.IMAGE,
            # 文档类型
            ...
            '.pptx': FileType.DOCUMENT, '.md': FileType.DOCUMENT,
            # 压缩文件
    ...
    async def save_file(self, file: UploadFile, user_id: str, is_public: bool = False) -> FileInfo:
        """保存文件到文件系统和数据库"""
        # 验证文件
        ...
        try:
            ...
            thumbnail_url = f"/api/v1/files/download/{file_id}_thumb" if thumbnail_path else None
    ...
    async def _process_image(self, image_path: str, file_id: str, extension: str) -> tuple[Optional[str], Dict[str, Any]]:
        """处理图片（压缩、格式转换和生成缩略图）"""
        metadata = {}
        thumbnail_path = None
        ...
        try:
            with Image.open(image_path) as img:
                ...
                thumbnail_full_path = os.path.join(self.upload_dir, "thumbnails", thumbnail_filename)
                thumbnail_img.save(thumbnail_full_path, format='JPEG', quality=80, optimize=True)
    ...
    async def get_user_files(self, user_id: str, file_type: Optional[FileType] = None, 
        ...
        # 获取分页数据
        query = query.order_by(UploadedFile.created_at.desc()).offset(offset).limit(size)
